...
import type { WikiArticle, WikiCuratorPreferences, WikiSelectionResult } from './types';
...
type CachedWikiPayload = {
  cachedAt: number;
  result: WikiSelectionResult;
};

function eventDTag(event: Event): string {
  const tag = event.tags.find((entry) => entry[0] === 'd' && typeof entry[1] === 'string');
  return (tag?.[1] || '').trim().toLowerCase();
}

function toAPointer(event: Event, dTag: string): string {
  return `${WIKI_KIND}:${event.pubkey}:${dTag}`;
}

async function collectWikiEventsBySlug(slug: string, relays: string[], timeoutMs: number = 4500): Promise<WikiArticle[]> {
  if (!slug || relays.length === 0) return [];
    ...
    await new Promise<void>((resolve) => {
      ...
      const sub = pool.subscribeMany(relays, [filter] as any, {
        onevent(event: Event, relayUrl?: string) {
          if (!eventValidator.validateEvent(event)) return;
          const dTag = eventDTag(event);
          if (dTag !== normalized) return;
...
async function fetchCuratorPreferences(relays: string[], curatorPubkey?: string): Promise<WikiCuratorPreferences> {
  ...
  try {
    const events = await pool.querySync(relays, filters as any, {
      maxWait: 3500,
...
export function selectBestWikiArticle(
  articles: WikiArticle[],
  preferences: WikiCuratorPreferences
): WikiSelectionResult {
...
export async function fetchWikiArticlesBySlug(
  slug: string,
  relays: string[],
  options?: { curatorPubkey?: string }
): Promise<WikiSelectionResult> {
...
export async function getCachedWikiArticlesBySlug(slug: string): Promise<WikiSelectionResult | null> {
  const normalizedSlug = normalizePlantSlug(slug);
  if (!normalizedSlug) return null;
  const payload = await getJson<CachedWikiPayload | null>(`${WIKI_CACHE_PREFIX}${normalizedSlug}`, null);
  if (!payload) return null;
  return payload.result;
}

export async function getWikiArticlesBySlugWithCache(
  slug: string,
  relays: string[],
  options?: { curatorPubkey?: string; forceRefresh?: boolean }
): Promise<{ result: WikiSelectionResult; fromCache: boolean; cachedAt?: number }> {
  const normalizedSlug = normalizePlantSlug(slug);
  if (!normalizedSlug) {
    return {
  ...
  const payload = await getJson<CachedWikiPayload | null>(cacheKey, null);
