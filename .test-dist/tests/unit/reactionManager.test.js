...
test('reactionManager ignores reactions with invalid signatures', () => {
    ...
    manager.processReactions([invalidReaction, validReaction]);
...
test('reactionManager tracks repost and zap counts from interactions', () => {
    ...
    const originalValidate = manager.isValidSignedEvent;
    manager.isValidSignedEvent = () => true;
    manager.processInteractions([repostA, repostB, zapA]);
    ...
    manager.isValidSignedEvent = originalValidate;
});
