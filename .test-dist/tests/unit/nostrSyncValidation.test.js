...
test('fetchPublicDiaries ignores events rejected by validator', async () => {
    ...
    const originalClose = SimplePool.prototype.close;
    const originalValidate = eventValidator.validateEvent.bind(eventValidator);
    SimplePool.prototype.querySync = async () => [invalidDiaryEvent, validDiaryEvent];
    SimplePool.prototype.close = () => { };
    eventValidator.validateEvent = (event) => event.id !== 'invalid-diary';
    try {
        ...
        SimplePool.prototype.close = originalClose;
...
test('fetchGrowmiesList ignores events rejected by validator and returns latest valid list', async () => {
    ...
    const originalClose = SimplePool.prototype.close;
    const originalValidate = eventValidator.validateEvent.bind(eventValidator);
    SimplePool.prototype.querySync = async () => [invalidGrowmiesEvent, validGrowmiesEvent];
    SimplePool.prototype.close = () => { };
    eventValidator.validateEvent = (event) => event.id !== 'invalid-growmies';
    try {
