...
function getNip46Bridge() {
    return globalThis.__WEEDOSHI_NIP46__ || null;
}
function normalizeNip46PairingState(input, fallbackPhase) {
    ...
    const phase = rawPhase === 'paired' || rawPhase === 'connected'
        ? 'paired'
        : rawPhase === 'pairing' || rawPhase === 'pending'
            ...
            : rawPhase === 'idle'
                ...
                : rawPhase === 'error' || rawPhase === 'failed'
                    ? 'error'
                    : fallbackPhase;
    const connectionUri = typeof obj.connectionUri === 'string'
        ? obj.connectionUri
...
class AuthManager {
    ...
    async loginWithBrowserSigner() {
        try {
            ...
            throw new Error(`Failed to connect browser signer: ${error instanceof Error ? error.message : ''}`);
        }
    }
    ...
    async approveNip46Pairing() {
        const bridge = getNip46Bridge();
        if (!bridge) {
            throw new Error('NIP-46 bridge not available');
        }
        const approveMethod = typeof bridge.approvePairing === 'function'
