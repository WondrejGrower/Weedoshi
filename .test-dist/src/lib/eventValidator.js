...
export class EventValidator {
    constructor() {
        Object.defineProperty(this, "invalidCount", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
        });
        Object.defineProperty(this, "validCount", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
    ...
    validateEvent(event) {
        try {
            ...
            if (!signatureValid) {
                this.invalidCount++;
                diagnostics.log(`❌ Invalid signature for event ${event.id.substring(0, 8)}...`, 'error');
                return false;
            }
            ...
            if (event.created_at > now + oneDayInSeconds) {
                diagnostics.log(`⚠️ Event ${event.id.substring(0, 8)}... has suspicious future timestamp`, 'warn');
            ...
            if (event.created_at < now - thirtyDaysInSeconds) {
                diagnostics.log(`⚠️ Event ${event.id.substring(0, 8)}... is very old (>30 days)`, 'warn');
                // Still accept, but log warning
            }
            // 3. Basic content validation
            if (event.kind === 1 && event.content.length > 50000) {
                diagnostics.log(`⚠️ Event ${event.id.substring(0, 8)}... has suspiciously long content (${event.content.length} chars)`, 'warn');
            }
            this.validCount++;
            // Log stats periodically
            if ((this.validCount + this.invalidCount) % 100 === 0) {
                diagnostics.log(`Event validation: ${this.validCount} valid, ${this.invalidCount} invalid`, 'info');
            }
            return true;
        }
        catch (error) {
            this.invalidCount++;
            diagnostics.log(`❌ Event validation error: ${error instanceof Error ? error.message : 'Unknown error'}`, 'error');
            return false;
        }
    }
    /**
