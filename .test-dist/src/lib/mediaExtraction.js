...
export function stripStandaloneMediaLines(content, mediaUrls) {
    if (!content)
        return '';
...
function classifyFromMime(url, mime) {
    if (!mime)
        return 'none';
...
export function parseMediaFromEventTags(event) {
...
async function headContentType(url, timeoutMs) {
...
function shouldProbeWithHead(url) {
...
export async function enhanceMediaWithHead(initial, timeoutMs = 1500) {
    ...
    const stillLinks = [];
    await Promise.all(initial.links.map(async (url) => {
